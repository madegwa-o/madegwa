...
export default function HomePage() {
    ...
    async function createProject(event: React.FormEvent) {
        ...
        if (!response.ok) {
            setError((await response.json()).error || "Unable to create project")
            return
        }
            ...
            <div className="mx-auto max-w-3xl w-full">
                ...
                {showCreate && (
                    <form
                        ...
                        className="mb-8 grid gap-4 rounded-lg border border-border bg-card p-5 sm:grid-cols-[1fr_auto_auto] sm:items-end"
                    >
                        <label className="grid gap-2 text-sm">
                            ...
                            <input
                                ...
                                className="rounded-md border border-input bg-background px-3 py-2 outline-none ring-primary focus:ring-2"
                            />
                        </label>
                        ...
                        <button className="rounded-md bg-secondary px-4 py-2 font-medium text-secondary-foreground">
                            Create
                        </button>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {projects.map((project) => (
                            ...
                            >
                                ...
                                <h2 className="mt-6 text-lg font-medium">{project.name}</h2>

                                <div className="mt-4 flex items-center gap-4 text-xs text-muted-foreground">
                  <span className="inline-flex items-center gap-1">
                    {project.visibility === "public" ? <Globe2 /> : <LockKeyhole />}
                      {project.visibility}
                  </span>
                                    <span className="inline-flex items-center gap-1">
                    <KeyRound />
                                ...
                                <div className="mt-5 border-t border-border pt-3 text-xs text-muted-foreground">
                                    {project.role === "admin" ? "Admin" : project.role} ·{" "}
